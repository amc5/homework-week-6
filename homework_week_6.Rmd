...
##Below is the chunk of code that I wrote using modules, book, internet to try to create the function that you asked for. I'm not sure how to check if it works because I don't have values to plug in (?).  The code below this is taken from the answer key of the book that I used to check/correct parts of my own code that I worked through.  I then worked through the code from the answer key to ensure that I understood everything that is going on.
...
##below is from book
z.prop.test() <- function(p1, n1, p2=NULL, n2=NULL, p0, alternative = "two.sided", conf.level = 0.95) #this sets up the function's arguments per the first set of instructions for the function
  {
  if(p2=NULL||n2=NULL){cat("One-samaple Z-test.\n")
  if((n1*p1<=5)||(n1*(1-p)<=5))#check if data fits with rules of thumb, adequate sample size?
    {warning("Warning: Sample may not be normally distributed to run the Z-test")
  }
    z<- (p1-p0)/sqrt(p0*(1-p0/n1)) #THIS IS THE Z TEST
    ci<- p1+c(-1,1*qnorm(conf.level+(1-conf.level)/2)*sqrt(p0*(1-p0/n1))) #this sets up the confidence interval for the Z test
  else{
    cat("Two-sample Z-test.\n")
    if(p1*n1<=5||n1*(1-p1)<=5||p2*n2<=5||n2*(1-p2<=5)) #this tests to see if the sample size is large enough to run the Z test
    {warning("Sample size may not be large enough for normal distribution")}
    p.star<- (p1*n1+p2*n2)/(n1+n2) #another value within the two-sample z-test, pooled proportion of probabilities
    z<- (p1-p2-p0)/sqrt(p.star*(1-p.star)*(1/n1+1/n2)) #THIS IS THE TWO SAMPLE Z TEST
    ci<- sort((p1-p2)+c(-1,1)*qnorm(conf.level+(1-conf.level)/2)*sqrt(p.star*(1-p.star)*(1/n1+1/n2))) #sets up confidence interval for two sample test
  }
  p<- pnorm(z) #whatever is left of the z statistic under normal distribution
  if(alternative=="greater"){
    p<- 1-p}
